...
class dmBotTest_Flytime : dmTestSuite_TestCase
{
	...
	string FireCadence(dmAISurvivorBase pawn, out bool fired)
	{
		...
		{
			//! Не готов. Различаем две причины по состоянию СТВОЛА, а не по m_SubPhase
			//! (тот сбрасывается при переходе фазы и не отличает «поднимаю» от «стреляный
			//! ствол»):
			//!  - IsWeaponReady() == true  → ствол заряжен, бот ещё поднимает оружие — ждём;
			//!  - IsWeaponReady() == false → ствол пуст/стрелян — перезаряжаем; false от
			//!    ReloadWeaponAI означает «перезаряжать нечем» (реально кончились патроны).
			if (!pawn.IsWeaponReady())
			{
				if (!pawn.ReloadWeaponAI())
					return "DONE";
			}
