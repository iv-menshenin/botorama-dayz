...
enum dmBotIntentsChannel
{
	...
	DRIVE   // вождение (газ/руль машиной)
};
...
class dmBotIntent
{
	...
	//! ВАЖНО: Fail() ставит И m_Failed, И m_Finished — поэтому IsFinished() истинно
	//! и для провала. Чтобы отличить успех от провала, проверяй IsFailed() ДО IsFinished().
	void Fail()
	{
		#ifdef DM_BOT_DEBUG_FSM
