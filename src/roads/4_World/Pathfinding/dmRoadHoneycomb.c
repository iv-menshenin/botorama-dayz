...
class dmRoadHoneycomb
{
	...
	private bool RaycastBlocked(float cx, float groundY, float cz)
	{
		...
		//! Два скрещённых горизонтальных луча У ЗЕМЛИ (~0.15 м, размах в клетку):
		//! ловят НИЗКИЕ (~0.2 м) тонкие стены/бордюры, мимо которых вертикальный
		//! луч проскальзывает. Любой хит здесь — препятствие (ровная дорога ниже).
		float h = groundY + 0.15;
