...
**Готча (проверено v3.80→v3.81) — знак ветра**: эмпирически пуля сносится в направлении,
**противоположном поперечной составляющей** `GetWind()` (дрейф в `−windPerp`, `latSigned` и
`windPerp` разноимённы). Т.е. ветер «дует туда, куда указывает −GetWind()», а `GetWind()` — это
направление, ОТКУДА дует (upwind). Компенсация должна целиться **в +GetWind()**:
`projected += wind * coef * t`; обучение ветер-коэфа — `wc -= rate * latSigned / scale` (где
`latSigned` = знаковый поперечный промах, `scale = windPerp * t`). Если поставить `-wind` (против
ветра) и `wc +=` — оба знака перепутаны, коэф уходит в 0 и остаётся боковой снос ~0.4 м.
Наблюдаемый фактор сноса (коэф при `wind * t`) ~0.21 — потолок `DM_WIND_COEF_MAX` поднят до 0.3.
