...
**Готча (v3.80→v3.82) — ветер как источник бокового сноса ненадёжен, заменён на геометрию.**
Эмпирически пуля сносится в направлении, **противоположном поперечной составляющей** `GetWind()`
(`latSigned` и `windPerp` разноимённы), т.е. `GetWind()` — направление, ОТКУДА дует (upwind), а
снос — в `−windPerp`. Знак `scale = windPerp * t` зависит от направления прицела (переворачивается
при смене стороны), из-за чего обучение ветер-коэфа (v3.71–v3.81) расходилось/застревало в 0.
**Фикс v3.82**: убрать ветер вообще — накапливать знаковый **угол** бокового промаха напрямую
(`latAngle = latSigned / along`, `along` всегда > 0 → знак стабилен) и доворачивать прицел на
противоположный угол (`lc -= rate * latAngle`, поворот направления по азимуту). Геометрия не зависит
от направления ветра и меряет фактический промах (чем бы он ни вызван — ветром/дулом/ошибкой).
