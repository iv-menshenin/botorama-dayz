...
## Правило мировых координат (Y = 0)

Для **любого мирового поля** (`Pos`, `From`, `To`, `Min`, `Max`): если `Y == 0.0`
(в т.ч. поле не передано — читается как 0.0), мост заменяет точку на
`SnapToGroundExactly(p)` (реальный уровень земли). Если `Y != 0.0` — точка используется
как есть (явная высота, без снапа).

- `spawn`/`spawnobj`/`setpos`/`observe` — `ResolveWorldPos(Pos)`.
- `scanbox` — `ResolveWorldPos(Min)` и `ResolveWorldPos(Max)`.
- `raycast` — `Y==0` → земля **+ высота глаз** (`DM_E2E_EYE_HEIGHT`); `Y!=0` → сырой луч.

Сценарии пишут `Y=0` (авто-земля) и не хардкодят высоту; явный `Y` — только когда нужна
конкретная высота (напр. рейкаст сквозь перила на 2-м этаже).
