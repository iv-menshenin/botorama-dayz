...
- **Остановка (обязательно после прогона)**: `pkill -x DayZServer` + проверить
  `pgrep -x DayZServer` == пусто. Сервер не оставлять работающим после тестов.
  Если запускал клиент-наблюдателя — `pkill -x DayZ_x64.exe`.
  **Готча**: `pkill -f DayZServer` сам себя убивает, когда команда запущена через
  `bash -c` и её собственная командная строка содержит литерал `DayZServer`
  (паттерн `-f` матчит собственную оболочку агента → «зависание» на таймауте).
  Используй `pkill -x` (точное имя процесса) или `pkill -f '\./DayZServer '`
  (экранированная точка → не самоматчится).
