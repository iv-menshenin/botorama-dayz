...
- **`dmBotIntent.Fail()` ставит И `m_Failed`, И `m_Finished`** — поэтому `IsFinished()` истинно и для провала. Чтобы отличить успех от провала — проверяй `IsFailed()` ДО `IsFinished()` (иначе ветка «не удалось» — мёртвый код). Если тебе безразлично «успех/провал» — можно единый `IsFinished() || IsExpired() || IsFailed()`.
